...
class GraphPlanRelaxed():
    ...
    def expand(self):
        ...
        """
        state = frozenset(self.graph.nodes) # current state is all the element in the graph (we just use literal)
        nodes_added = 0
        # loop over possible usefull operators
        for operator in self.possible_operators:
            if operator.preconditions.issubset(state): # check if the action is possible 
    ...
    def select_actions_backward(self, goals):
        ...
        for goal in goals:
            if not (goal in complete_goals): # check if the goal is not already done
                actions = list(self.pred[goal].keys()) # possible actions to attempt the goals
                ...
                complete_goals = complete_goals.union(action.add_effects) # add effects of the action to the completed goals
                set_actions = set_actions.union(frozenset([action])) # add action to the set of action
